...
Lockdown=as.Date("2020-03-22")
Ph2=as.Date("2020-06-04")
ScOpen=as.Date("2020-08-20")
Ph3=as.Date()
Msk=as.Date("2020-07-11")
p1=ggplot(df1, aes(x=Date, y=Cases_ma)) +
  geom_rect(xmin = df1$Date[173], xmax = df1$Date[178], ymin = -500, ymax = max(df1$Cases_Total)+500,alpha = .05,fill="lightgreen")+
  geom_point(size=0.5)+
  geom_line(data=df1, aes(x = Date, y = Cases_ma, colour = "Actual Values"))+
  geom_line(data=results, aes(x = Date, y = PredNN, colour = "NN Predictions"))+
  geom_vline(xintercept=Lockdown, linetype=4,colour="red")+
  geom_text(x=Lockdown, y=1800, label="Lockdown",colour="red",size=geom.text.size)+
  ...
  geom_vline(xintercept=as.numeric(df1$Date[133]), linetype=4,colour="red")+
  geom_text(x=as.numeric(df1$Date[133]), y=1500, label="Mask Mandate",colour="red",size=geom.text.size)+
  geom_text(x=as.numeric(df1$Date[173]), y=1500, label="Schools Open",colour="green",size=geom.text.size)+
  geom_vline(xintercept=as.numeric(df1$Date[173]), linetype=4,colour="green")+
  geom_vline(xintercept=as.numeric(df1$Date[178]), linetype=4,colour="green")+
  geom_text(x=as.numeric(df1$Date[199]), y=1000, label="Phase 3 Reopening",colour="green",size=geom.text.size)+
  geom_vline(xintercept=as.numeric(df1$Date[193]), linetype=4,colour="green")+
  geom_text(x=as.numeric(df1$Date[270]), y=1000, label="Modified Phase 2",colour="green",size=geom.text.size)+
  geom_vline(xintercept=as.numeric(df1$Date[270]), linetype=4,colour="green")+
  geom_text(x=as.numeric(df1$Date[310]), y=1100, label="Vaccinations Started",colour="red",size=geom.text.size)+
  geom_vline(xintercept=as.numeric(df1$Date[310]), linetype=4,colour="red")+
  geom_text(x=as.numeric(df1$Date[368]), y=1100, label="Phase 3",colour="green",size=geom.text.size)+
  geom_vline(xintercept=as.numeric(df1$Date[368]), linetype=4,colour="green")+
  scale_x_date(breaks = "1 month",date_labels="%b")+
  ylab("Daily Cases")+
